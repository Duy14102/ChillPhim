...
function AdminControl({ SearchMainFilm, currentPage3, state, setState, axios, toast, ToastUpdate, useRef, useEffect }) {
    ...
    useEffect(() => {
        callAccount(state.searchAccMain ? state.searchAccMain : null)
    }, [])
    ...
    return (
        <div className="adminControl">
            <div className="upperControl">
                <SearchMainFilm search={state.searchAccMain} setState={setState} callBacks={callAccount} useEffect={useEffect} type={3} />
            ...
            <table>
                ...
                <tbody>
                    {state.listAccounts.length < 1 ? (
                        ...
                        state.listAccounts.filter((item) => item.username !== "admin").map((i) => {
                            return (
                                <tr key={i._id}>
                                    <td>{i.username}</td>
            ...
            {state.listAccounts.length > 0 ? (
                <Pagination searchEnale={state.searchAccMain} pageCount={state.pageCount3} callBack={callAccount} currentPage={currentPage3} />
            ) : null}
        </div>
    )
}
